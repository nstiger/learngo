...
func main() {
	...
	//The types of the column will be inferred.
	...
	//Filter the dataframe again, but only select cut the 
	//sepal_width and species column.
	versicolorDF = irisDF.Filter(filter).Select([]string{"sepal_width", "species"})
	fmt.Println(versicolorDF)
	fmt.Println("--------------sepal_width, species-------------")
	...
	fmt.Println(versicolorDF)
	fmt.Println("---------------------first three results--------------------")
}
